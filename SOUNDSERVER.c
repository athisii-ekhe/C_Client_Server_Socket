   ...
   first:
      ...
      start:
         ...
         if (strncmp(buffer, "1\n", 2) == 0)
         {
            ...
            n = createUser(clientData, id, password);
            if (n == 1)
            {
         ...
         else if (strncmp(buffer, "2\n", 2) == 0)
         {
            ...
            n = login(clientData, id, password);
            if (n == 1)
            {
               //Displays id and password in console (Testing)
               bzero(temp, sizeof(temp));
               strcat(temp, "SERVER: WELCOME BACK, ");
               strcat(temp, id);
               strcat(temp, "!\n");
               fputs("NEW LOGIN\n", stdout);
               write(confd, temp, strlen(temp));
            ...
            {
               fputs("WRONG ID\n", stdout);
               write(confd, login_err_msg, strlen(login_err_msg));
               close(confd);
               goto first;
            }
         }
